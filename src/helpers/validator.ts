...
import { validationRuleForCash, validationRuleForCheque, validationRuleForUPI, validationRuleForBankTransfer } from "./../modules/admin/payment/paymentValidation"
...
Validator.registerAsync('validatePaymentMethod', function (value: any, attribute, req: any, passes) {
  const key: any = Object.keys(value)[0]
  let body: any = Object.values(value)[0]
  let rules = {}
  if (key === constants.paymentMethod.cash) {
    body = Object.values(value)[0];
    rules = validationRuleForCash
  }
  else if (key === constants.paymentMethod.bankTransfer) {
    rules = validationRuleForBankTransfer;
    body = body.internetBanking
  }
  else if (key === constants.paymentMethod.cheque) {
    rules = validationRuleForCheque;
    body = body.chequeDetails
  }
  else if (key === constants.paymentMethod.upi) {
    rules = validationRuleForUPI;
    body = Object.values(value)[0]
  }
  validator(body, rules, {},
    (err: any, status: boolean) => {
      if (!status) {
        const errorMsg: any = Object.values(err.errors)[0]
...
Validator.register(
  "checkUPI",
  (value: any) => UPIRegex.test(value) === true,
  constants.message.invalidUPI
);
...
Validator.registerAsync(
  "checkGST",
  async function (value, attribute, req, passes) {
    const gstExists = await Company.exists({ "gst.value": value });
    if (!gstExists) {
      passes(false, constants.message.invalidGSTNum);
    } else {
      passes();
    }
  },
...
Validator.registerAsync(
  ...
  async function (value: any, attribute, req, passes) {
    ...
    if (!regex.test(value)) {
      passes(false, constants.message.invalidAccNum);
    } else {
      try {
        const existingBank = await Bank.findOne({
          "account_no.value": value,
          isDeleted: false,
          status: true,
        });
        if (existingBank) {
          passes(false, constants.message.AccNumExists);
        } else {
          passes();
        }
      } catch (error) {
        passes(false, "Error checking account number uniqueness.");
      }
    }
  },
  constants.message.invalidAccNum
  ...
  async function (value: any, attribute, req, passes) {
    const privileges: any = new Map(
      [
        ...
        [
          constants.privileges.catalogue_management,
          [
            constants.rights.read,
            constants.rights.write,
            constants.rights.delete,
        ...
        [
          constants.privileges.order_management,
          [
            constants.rights.read,
            constants.rights.write,
            constants.rights.delete,
...
const bankAccountNumberRegex: any = /^\d{9,18}$/;

Validator.register(
  "checkBankAccountNumber",
  (value) => bankAccountNumberRegex.test(bankAccountNumberRegex) === true,
  "Invalid bank account number. It should be between 9 to 18 digits."
)
